...
int main(int argc, char* argv[]) {
	...
	if (argc > 1 && (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help")) {
		...
		std::cout << "Default output file path: ./default_file_name.raw" << std::endl << std::endl;
		std::cout << long_program_desc << std::endl;
		return 0;
	}

	std::string out_file_path = "./default_file_name.raw";
	...
	else
	{
		std::cout << "The current file path is not entered, "
			...
			<< "use the input parameters to set it." << std::endl << std::endl;
	}
	...
	do {
			...
			if (camera_descs.size() > 0)
			{
				...
				if (camera)
				{
					...
					camera->setStatisticInfoCallback([&statistic_info](const dvsense::DsStatisticInfo info)
													 { statistic_info = info; });
					// camera->setFileCacheTime(5);
					std::shared_ptr<dvsense::CameraTool> bias = camera->getTool(dvsense::ToolType::TOOL_BIAS);
					bias->setParam("bias_diff_on", 30);
					bias->setParam("bias_diff_off", 30);
					camera->start();
				}
			}
			else
		...
		if (!display.empty()) {
			cv::putText(display, std::to_string(statistic_info.bandwidth_byte / 1024.0 / 1024.0) + " MB/s",
				cv::Point(10, 15), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
			cv::putText(display, std::to_string(statistic_info.events_count / 1000000.0) + " MEv/s",
